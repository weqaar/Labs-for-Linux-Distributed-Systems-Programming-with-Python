...
def load_checkpoint(root: Path | None = None) -> DeploymentCheckpoint:
    ...
    canary_path = actual_root / "deploy/relay-canary.yaml"
    environments_path = actual_root / "deploy/relay-environments.yaml"
    kind_path = actual_root / "deploy/kind-cluster.yaml"
    strategies_path = actual_root / "deploy/release-strategies.yaml"

    dockerfile = dockerfile_path.read_text(encoding="utf-8")
    manifest_documents = list(yaml.safe_load_all(manifest_path.read_text(encoding="utf-8")))
    if len(manifest_documents) != 6:
        raise ArtifactValidationError(
            "deployment manifest must contain identity, workload, traffic, and availability objects"
        )
    ...
    service = _expect_mapping(manifest_documents[2], "service")
    disruption_budget = _expect_mapping(manifest_documents[3], "pod disruption budget")
    autoscaler = _expect_mapping(manifest_documents[4], "horizontal pod autoscaler")
    ingress = _expect_mapping(manifest_documents[5], "ingress")
    rollout = _expect_mapping(yaml.safe_load(rollout_path.read_text(encoding="utf-8")), "rollout")
    canary = tuple(
        _expect_mapping(document, "canary document")
        for document in yaml.safe_load_all(canary_path.read_text(encoding="utf-8"))
    )
    environments = tuple(
        _expect_mapping(document, "environment document")
        for document in yaml.safe_load_all(environments_path.read_text(encoding="utf-8"))
    )
    kind_cluster = _expect_mapping(yaml.safe_load(kind_path.read_text(encoding="utf-8")), "kind")
    release_strategies = _expect_mapping(
        yaml.safe_load(strategies_path.read_text(encoding="utf-8")), "release strategies"
    )
...
def validate_checkpoint(checkpoint: DeploymentCheckpoint) -> None:
    ...
    _validate_cluster_traffic(checkpoint)
    _validate_rollout(checkpoint.rollout, checkpoint.image)
    _validate_release_variants(checkpoint)
...
def _validate_dockerfile(dockerfile: str) -> None:
    ...
    if 'CMD ["python", "-m", "lab_32_container_deploy.relay_api"]' not in dockerfile:
...
def _validate_deployment(deployment: dict[str, Any], service_account: dict[str, Any]) -> None:
    ...
    if int(spec.get("replicas", 0)) < 2:
        raise ArtifactValidationError("deployment must keep at least two replicas")
    ...
    startup = _probe_path(container, "startupProbe")
    if {readiness, liveness} != {"/readyz", "/livez"}:
        raise ArtifactValidationError("readiness and liveness must use distinct health paths")
    if startup != "/livez":
        raise ArtifactValidationError("startup probe must protect slow process initialization")
    if not pod_spec.get("topologySpreadConstraints"):
        raise ArtifactValidationError("deployment must spread replicas across failure domains")
...
def _validate_cluster_traffic(checkpoint: DeploymentCheckpoint) -> None:
    if checkpoint.service.get("kind") != "Service":
        raise ArtifactValidationError("relay traffic must use a Kubernetes Service")
    service_spec = _expect_mapping(checkpoint.service.get("spec"), "service spec")
    if service_spec.get("type") != "ClusterIP":
        raise ArtifactValidationError("relay service must remain internal behind ingress")
    if checkpoint.disruption_budget.get("kind") != "PodDisruptionBudget":
        raise ArtifactValidationError("relay must constrain voluntary disruptions")
    if checkpoint.autoscaler.get("kind") != "HorizontalPodAutoscaler":
        raise ArtifactValidationError("relay must declare horizontal scaling")
    hpa_spec = _expect_mapping(checkpoint.autoscaler.get("spec"), "autoscaler spec")
    if int(hpa_spec.get("minReplicas", 0)) < 2:
        raise ArtifactValidationError("autoscaler must preserve two serving replicas")
    if checkpoint.ingress.get("kind") != "Ingress":
        raise ArtifactValidationError("relay must expose an ingress")
    ingress_spec = _expect_mapping(checkpoint.ingress.get("spec"), "ingress spec")
    if ingress_spec.get("ingressClassName") != "nginx":
        raise ArtifactValidationError("relay ingress must use the Nginx controller")


def _validate_release_variants(checkpoint: DeploymentCheckpoint) -> None:
    if [document.get("kind") for document in checkpoint.canary] != [
        "Deployment",
        "Service",
        "Ingress",
    ]:
        raise ArtifactValidationError("canary must include a deployment, service, and ingress")
    metadata = _expect_mapping(checkpoint.canary[2].get("metadata"), "canary ingress metadata")
    annotations = _expect_mapping(metadata.get("annotations"), "canary annotations")
    if annotations.get("nginx.ingress.kubernetes.io/canary-weight") != "10":
        raise ArtifactValidationError("canary ingress must begin with ten percent traffic")
    namespaces = {
        _expect_mapping(document.get("metadata"), "namespace metadata").get("name")
        for document in checkpoint.environments
        if document.get("kind") == "Namespace"
    }
    if namespaces != {"relay-staging", "relay-prod"}:
        raise ArtifactValidationError("staging and production need isolated namespaces")
    if checkpoint.kind_cluster.get("kind") != "Cluster":
        raise ArtifactValidationError("local Kubernetes must have a kind cluster definition")
    if checkpoint.release_strategies.get("kind") != "RelayReleaseStrategies":
        raise ArtifactValidationError("rolling, canary, and blue-green evidence is required")
    strategies = _expect_mapping(checkpoint.release_strategies.get("spec"), "strategies")
    if set(strategies) != {"rolling", "canary", "blueGreen"}:
        raise ArtifactValidationError("release evidence must cover rolling, canary, and blue-green")
