...
```
src/lab_32_container_deploy/    the package
...
- Keep the local runtime planner side-effect free. The CLI should print plans,
  not change the host.
- Keep Docker and Kubernetes SDK calls behind typed adapters. Deterministic
  gates inject fakes and must not require a Docker socket, kubeconfig or cluster.
- Docker smoke checks must use an image digest, a bounded wait and cleanup in a
  `finally` block. Kubernetes promotion must patch by digest and observe the
  Deployment generation and available replicas.
- Preserve the complete serving path: startup, readiness and liveness probes,
  two stable replicas, ClusterIP Service, Nginx Ingress, HPA,
  PodDisruptionBudget and topology spread.
- Staging and production stay in separate namespaces. The local kind
  configuration retains two workers.
- Canary traffic begins at ten per cent and uses a second Deployment and
  Service. Release strategy evidence keeps rolling, canary and blue-green
