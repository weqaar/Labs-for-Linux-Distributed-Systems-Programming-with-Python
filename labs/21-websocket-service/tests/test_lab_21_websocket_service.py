...
def test_graphql_mutation_query_authorization_and_cursor_pagination() -> None:
    api = RelayGraphQL()
    mutation = """
      mutation {
        first: submitTask(id: "task-17", action: "index") { id state }
        second: submitTask(id: "task-18", action: "deliver") { id state }
      }
    """

    denied = api.execute(mutation)
    created = api.execute(mutation, scopes=frozenset({"tasks:write"}))
    first_page = api.execute(
        "{ tasks(first: 1) { edges { cursor node { id } } pageInfo { endCursor hasNextPage } } }",
        scopes=frozenset({"tasks:read"}),
    )
    first_data = cast(dict[str, Any], first_page.data)
    cursor = first_data["tasks"]["pageInfo"]["endCursor"]
    second_page = api.execute(
        "query($after: String) { tasks(first: 1, after: $after) { edges { node { id } } } }",
        variables={"after": cursor},
        scopes=frozenset({"tasks:read"}),
    )

    assert denied.errors and denied.errors[0].message == "missing required scope: tasks:write"
    assert created.errors is None
    second_data = cast(dict[str, Any], second_page.data)
    assert first_data["tasks"]["edges"][0]["node"]["id"] == "task-17"
    assert first_data["tasks"]["pageInfo"]["hasNextPage"] is True
    assert second_data["tasks"]["edges"][0]["node"]["id"] == "task-18"


def test_graphql_loader_batches_task_lookups() -> None:
    api = RelayGraphQL()
    for task_id in ("task-17", "task-18"):
        result = api.execute(
            'mutation($id: ID!) { submitTask(id: $id, action: "index") { id } }',
            variables={"id": task_id},
            scopes=frozenset({"tasks:write"}),
        )
        assert result.errors is None

    loader = api.batch_loader()
    records = loader.load_many(("task-17", "task-18", "task-19"))

    assert [record.id if record else None for record in records] == [
        "task-17",
        "task-18",
        None,
    ]
...
def test_graphql_subscription_replays_events_after_sequence() -> None:
    api = RelayGraphQL()
    for task_id in ("task-17", "task-18"):
        api.execute(
            'mutation($id: ID!) { submitTask(id: $id, action: "index") { id } }',
    ...
    async def collect() -> list[dict[str, object]]:
        stream = await api.subscribe(
            "subscription { taskEvents(after: 1) { id state } }",
            scopes=frozenset({"tasks:read"}),
        )
        return [cast(dict[str, object], result.data) async for result in stream]

    assert asyncio.run(collect()) == [{"taskEvents": {"id": "task-18", "state": "QUEUED"}}]


def test_graphql_runs_over_a_real_websocket_connection() -> None:
    api = RelayGraphQL()
    frames = asyncio.run(
        graphql_websocket_round_trip(
            api,
    ...
    payload = cast(dict[str, Any], frames[1]["payload"])
    assert payload["data"]["submitTask"] == {
        "id": "task-17",
        "state": "QUEUED",
    }
