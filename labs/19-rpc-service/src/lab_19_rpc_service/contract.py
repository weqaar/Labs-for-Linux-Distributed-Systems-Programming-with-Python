...
class TaskSubmission:
    ...
    def __post_init__(self) -> None:
        ...
        _require_text(self.target, field_name="target", max_length=MAX_TARGET_LENGTH)
    ...
    def from_mapping(cls, payload: Mapping[str, Any]) -> TaskSubmission:
        ...
        return cls(
            id=_require_string(payload.get("id"), field_name="id", max_length=MAX_TASK_ID_LENGTH),
            action=action,
            target=_require_string(
                payload.get("target"), field_name="target", max_length=MAX_TARGET_LENGTH
            ),
...
def parse_timestamp(value: Any, *, field_name: str) -> datetime:
    ...
    text = _require_string(value, field_name=field_name, max_length=MAX_TIMESTAMP_LENGTH)
    try:
...
def _require_task_id(value: str) -> None:
    if len(value) > MAX_TASK_ID_LENGTH:
        raise ContractError(f"id must be at most {MAX_TASK_ID_LENGTH} characters")
...
def _require_text(value: str, *, field_name: str, max_length: int | None = None) -> None:
    if not value.strip():
        raise ContractError(f"{field_name} must not be empty")
    if max_length is not None and len(value) > max_length:
        raise ContractError(f"{field_name} must be at most {max_length} characters")


def _require_string(value: Any, *, field_name: str, max_length: int | None = None) -> str:
    if not isinstance(value, str):
        raise ContractError(f"{field_name} must be a string")
    _require_text(value, field_name=field_name, max_length=max_length)
    return value
