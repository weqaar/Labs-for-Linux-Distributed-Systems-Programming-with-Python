"""Tests for the lab_16_framed_protocol package."""

from __future__ import annotations

import pytest

from lab_16_framed_protocol import (
    BoundedRingBuffer,
    BufferFullError,
    FrameTooLargeError,
...
def test_ring_buffer_wraps_without_changing_fifo_order() -> None:
    buffer = BoundedRingBuffer(8)
    buffer.write(b"abcdef")
    assert buffer.read(5) == b"abcde"

    buffer.write(b"123456")

    assert len(buffer) == 7
    assert buffer.peek(7) == b"f123456"
    assert buffer.read(7) == b"f123456"
    assert buffer.free == buffer.capacity


def test_ring_buffer_rejects_overflow_as_one_atomic_write() -> None:
    buffer = BoundedRingBuffer(5)
    buffer.write(b"abc")

    with pytest.raises(BufferFullError, match="2 bytes free"):
        buffer.write(b"def")

    assert buffer.read(3) == b"abc"


def test_ring_buffer_rejects_invalid_ranges() -> None:
    buffer = BoundedRingBuffer(4)
    buffer.write(b"ab")

    with pytest.raises(UnexpectedEOFError, match="requested bytes"):
        buffer.peek(3)
    with pytest.raises(ValueError, match="negative"):
        buffer.peek(-1)


def test_reader_drains_many_frames_through_one_frame_capacity() -> None:
    frames = [encode_frame(bytes([number])) for number in range(10)]
    reader = IncrementalFrameReader(maximum_frame_size=1, buffer_capacity=5)

    assert reader.feed(b"".join(frames)) == [bytes([number]) for number in range(10)]
    assert reader.finish() == []


def test_reader_requires_capacity_for_one_complete_maximum_frame() -> None:
    with pytest.raises(ValueError, match="one header"):
        IncrementalFrameReader(maximum_frame_size=8, buffer_capacity=11)
