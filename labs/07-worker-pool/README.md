...
Relay checkpoint seven moves `relay` from a single-task scaffold to a
bounded worker pool.
...
This lab models the queueing side of the service. A typed queue lease keeps
a task invisible for a visibility timeout, redelivers it if a worker dies
mid-task, and lets the pool drain current work after a represented SIGTERM.
...
```
src/lab_07_worker_pool/    the package
