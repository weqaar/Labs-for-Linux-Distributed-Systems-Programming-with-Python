...
```
src/lab_05_object_oriented_design/    the package
tests/                the test suite
```

## Conventions

- Keep application services dependent on `TaskRepository` and handler
  interfaces. SDK clients belong in adapters, not in domain objects.
- Preserve the beginner object checks for type, identity, public attributes,
  bound methods, `self`, MRO, and standard special-method behavior.
- Preserve immutable slot-backed task values and return snapshots rather than
  exposing mutable repository dictionaries or audit lists.
- Decorators must preserve wrapped metadata. Metaclass registration is limited
  to handler discovery and must expose a read-only registry.
